-- Recompute trending cache (posts + videos) for a given window (hours)
...
BEGIN
  -- drop rows only for this window
  DELETE FROM trending_cache WHERE window_hours = p_window_hours;

  WITH
  -- ===== Per-object aggregates (latest engagement time + counts)
  post_like_agg AS (
  ...
  video_rows AS (
    SELECT
      ...
      v.id                                           AS object_id,
      v.user_id                                      AS creator_id,  -- <- IMPORTANT
      v.title                                        AS title,
    ...
    WHERE v.created_at >= since_ts AND v.user_id IS NOT NULL AND v.user_id IS NOT NULL
  ),
  all_rows AS (
  ...
  SELECT
    ...
    now()
  FROM scored s
  ORDER BY s.score DESC;  -- <<<<<< no ambiguity
END;
$$;
