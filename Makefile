...
.PHONY: setup db-up migrate api health smoke fmt lint type
...
migrate:
	...
	# Add any additional migration files here in order
...
fmt:
	find . -type f -name '*.sql' -print0 | xargs -0 -I{} sh -c "sed -i 's/[ \t]\+$$//' '{}'"
	pnpm -w prettier --write "**/*.{js,ts,tsx,json,md}"
